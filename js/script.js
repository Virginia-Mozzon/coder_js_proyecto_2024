...
const listaHusosHorariosPorPais = [
    ...
    ["UTC -10", ["Hawái (EE. UU.)", "Francia (Polinesia)", "Tuvalu"]],
    ["UTC -9", ["Alaska (EE. UU.)", "Islas Gambier"]],
    ["UTC -8", ["California (EE. UU.)", "Canadá", "México"]],
    ["UTC -7", ["Arizona (EE. UU.)", "Mendoza (Argentina)"]],
    ["UTC -6", ["México", "Guatemala", "Honduras", "Belice", "Costa Rica", "El Salvador"]],
    ["UTC -5", ["Estados Unidos", "Colombia", "Perú", "Venezuela", "Bahamas", "Cuba", "Ecuador", "Haití", "Jamaica", "Panamá", "Bahamas"]],
    ["UTC -4", ["República Dominicana", "Cuba", "Puerto Rico", "Antigua y Barbuda", "Barbados", "Chile", "Dominica", "Granada", "San Cristóbal y Nieves", "San Vicente y las Granadinas", "Santa Lucía", "Trinidad y Tobago", "Bolivia"]],
    ...
    ["UTC +3", ["Arabia Saudita", "Moscú (Rusia)", "Bielorrusia", "Etiopía", "Baréin", "Catar", "Comoras", "Etiopía", "Jordania", "Kuwait", "Siria", "Somalia", "Turquía", "Uganda", "Yibuti"]],
    ...
    ["UTC +8", ["China", "Malasia", "Singapur", "Brunéi"]],
    ["UTC +9", ["Japón", "Corea del Sur", "Mongolia", "Palaos", "Timor Oriental"]],
    ["UTC +10", ["Australia (AEST)", "Papúa Nueva Guinea", "Micronesia"]],
    ["UTC +11", ["Islas Salomón", "Nueva Caledonia", "Vanuatu"]],
    ["UTC +12", ["Fiyi", "Islas Gilbert", "Kiribati", "Nauru"]],
    ["UTC +13", ["Tonga", "Islas Cook"]],
    ["UTC +14", ["Línea Internacional de Cambio de Fecha"]],
    ["UTC -7 (PST)", ["California (EE. UU.)", "Oregón (EE. UU.)"]],
    ["UTC -6 (CST)", ["Illinois (EE. UU.)", "México"]],
    ["UTC -5 (EST)", ["Nueva York (EE. UU.)", "Toronto (Canadá)"]],
    ["UTC +10 (AEST)", ["Nueva Gales del Sur (Australia)", "Queensland (Australia)"]],
    ["UTC +9:30 (ACST)", ["Australia del Sur (Australia)", "Territorio del Norte (Australia)"]],
    ["UTC +8 (AWST)", ["Australia Occidental (Australia)", "Brunéi"]],
    ["UTC +3 (AEDT)", ["Sydney (Australia)", "Melbourne (Australia)"]],
...
class Pais {
    constructor (nombre, husoHorario, feriados) {
        this.nombre = nombre;
        this.husoHorario = husoHorario;     //Huso horario correspondiente a ese pais.
        this.feriados = feriados;           //Lista con los feriados o días no laborables de ese país.
    }
}


class HusoHorario {
    constructor (huso, paises, factorSumador) { 
        this.huso = huso;
        this.paises = paises;               //Lista de paises que se manejan con ese huso horario
        this.factorSumador = factorSumador; //Entero que representa la cantidad de horas a sumar con respecto al Greenwich (UTC 0)
    }
}
...
function crearHusosHorarios(){
    let final = [];
    
    for(sublista of listaHusosHorariosPorPais){
        let husoAtributo = sublista[0];
        let paisesAtributo = sublista[1];
        let factorSumador;

        let husoSpliteado = husoAtributo.split(" ");
        ...
        let huso = new HusoHorario(husoAtributo, paisesAtributo, factorSumador);
        final.push(huso);
    }
    return final;
}

const listaHusos = crearHusosHorarios()


function crearListaPaises() {
    let final = [];

    for(lista of listaFeriados) {
        let nombreAtributo = lista[0];
        let feriadosAtributo = lista[1];
        let husoAtributo
        let listoHusoHorario = false

        for(huso of listaHusos){
            for(paisListado of huso.paises){
                if (nombreAtributo === paisListado){
                    husoAtributo = huso.husoHorario;
                    listoHusoHorario = true
                    break;
                }
            }
            if(listoHusoHorario){
        ...
        let nuevoPais = new Pais(nombreAtributo, husoAtributo, feriadosAtributo);
        final.push(nuevoPais);
    }
    return final;
}
...
function retornarIndicePais(paisIngresado) {
    let contador = 0
    for(pais of paisesStrings){
        let paisLowerCase = pais.toLowerCase();

        if(paisIngresado.toLowerCase() === paisLowerCase){
            return contador;
        }
        contador += 1;
    }
...
function solicitarUnPais(){
    let nombrePais = prompt("Ingrese un pais.");
    
    while(esPaisValido(nombrePais) === false){
        nombrePais = prompt("Por favor, ingresar un país válido.")
    }
    let indicePais = retornarIndicePais(nombrePais);
    nombrePais = paisesStrings[indicePais];
