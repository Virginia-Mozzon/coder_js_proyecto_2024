...
export function seleccionarPais(nombrePais){
...
export function crearListaPaises() {
    let final = [];
    let huso;

    for(let i = 0; i < listaHusosHorariosPorPais.length; i++){
        huso = listaHusosHorariosPorPais[i][0];
        let paises = listaHusosHorariosPorPais[i][1];
        for(let pais of paises){
            final.push(new Pais(pais, huso));
        }
    }
...
export const listaPaises = crearListaPaises();